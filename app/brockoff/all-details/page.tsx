...
import { allColumns, allDetailsTableColumns } from '@/lib/columns';
...
export default function BrockoffAllDetailsPage() {
  ...
  const editableColumns = allColumns.filter(c => c.editable !== false && c.key !== 'created_at').map(c => c.key);

  const data = useMemo(() => rawData.filter(r => r.lot?.startsWith('B')), [rawData]);

  const baseFilteredData = data.filter(record => record.purchase_date);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold">Brockoff - Edit All Group Details</h2>
      ...
      <DataTable
        columns={allDetailsTableColumns}
        ...
        onEdit={(record) => setEditingRecord({ ...record, _fields: editableColumns })}
      ...
      {editingRecord && (
        <EditModal
          ...
          fields={(editingRecord._fields as string[]) || editableColumns}
          onSave={handleSave}
