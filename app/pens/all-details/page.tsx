...
export default function PenAllDetailsPage() {
  ...
  const [form, setForm] = useState({ pen_name: '', pen_square_feet: '', pen_type: 'Open Lot', bunk_space_ft: '', pre_ship: false });
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      pen_square_feet: form.pen_square_feet ? Number(form.pen_square_feet) : null,
      pen_type: form.pen_type,
      bunk_space_ft: form.bunk_space_ft ? Math.floor(Number(form.bunk_space_ft)) : null,
      pre_ship: form.pre_ship,
    };
    const success = await update(editingPen.id, payload);
    if (success) {
      setEditingPen(null);
      setForm({ pen_name: '', pen_square_feet: '', pen_type: 'Open Lot', bunk_space_ft: '', pre_ship: false });
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold">Edit All Pen Details</h2>
      ...
      {editingPen && (
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="font-semibold mb-4">Edit All Details</h3>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <input type="number" value={form.pen_square_feet} onChange={e => setForm({ ...form, pen_square_feet: e.target.value })} className="w-full border rounded px-3 py-2" />
            </div>
            <div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredPens.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-500">No pens found.</td></tr>
            ) : (
